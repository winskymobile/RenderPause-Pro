...
其实，**自己把窗口最小化 / 隐藏**，往往就能把闲时 GPU 压下来——系统不再给那些看不见的图层做持续合成。  
但很多人的习惯是：切到别的应用继续干活，很少会主动点黄灯或 `Cmd+H`。窗口一多、一遮挡，**人已经不用了，GPU 却还在干活**。

RenderPause Pro 就是为这个习惯差距做的：你照常切换窗口；它只在名单应用**真闲置且被完全挡住**时自动隐藏，需要时再切回即恢复。  
日常写代码、看网页、切窗口，和以前一样——不改你的用法，只补上「忘了最小化」那一环。
...
| Pencil 等「挂着不用也费电」 | 静止窗口仍占渲染 / 合成路径 |
...
打开 **「活动监视器」→ GPU / WindowServer**，在「窗口很多但你在干别的事」时看是否偏高。  
偏高，才值得用本工具；不高，就不必硬加应用。

### 2. 只加真凶

偏好设置 → **应用名单** → **添加**，只加入你怀疑后台仍费 GPU 的应用。  
**不必全加**——没进名单的应用，本工具完全不碰；加多了只会增加无谓打扰。
...
2. **只加真凶**：添加你怀疑费 GPU 的应用（仅名单内生效；日常其他 App 完全不动）
3. 确认 **启用监控**，按需改 **触发时间**（默认 30 秒）
4. 照常工作：名单应用被完全挡住并超过阈值后，才会自动隐藏
5. 需要时再切回 → 立即恢复显示

**菜单栏：** 状态 · 暂停/恢复监控 · 名单（✓ / 状态字）· **添加应用** · 打开窗口 · 退出  
